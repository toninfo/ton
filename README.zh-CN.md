...
# 若 raw.githubusercontent.com 返回 403，改用 jsDelivr 镜像：
curl -fsSL https://cdn.jsdelivr.net/gh/toninfo/ton@main/install.sh | bash

# GitHub API 限流时可固定版本（可选）：
...
始终优先在 **可写项目目录** 里跑 —— ton 会创建 `<workspace>/.ton/`，
编码代理也会在那里改文件。不要用 `sudo` 跑。

```bash
ton setup --api-key …             # 一次：写入 ~/.config/ton/llm.env
ton doctor                        # 扫描代理并打印配置路径
cd /path/to/your/project          # 你的用户可写目录
ton                               # 或：ton -w /path/to/your/project
```

若从 root 拥有的路径（如 `/home/work`）启动且未带 `-w`，
会 **回退** 到 `~/ton-workspace` 并打印提示 —— 仍能用，但
在意文件落点时请用 `-w` 指向真实仓库。

可选：固定 driver；未设置 → 扫描 opencode/claude/agent 并选择
