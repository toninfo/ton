"""
Nago's layered memory model. The long-term layer stores only durable facts
that should still be remembered in future interactions.
...
3. long_term: curated facts retained across compression and restarts.
...
_EXPLICIT_RE = re.compile(
    ...
    r"我叫|叫我|我是|我喜欢|我不喜欢|别再|不要再|禁止|请记得)",
    re.IGNORECASE,
)
...
class LongTermMemory:
    ...
    def maybe_promote_from_user(self, text: str) -> bool:
        """Promote a user message with an explicit memory signal to a candidate fact."""
        text = _norm_text(text)
        if not text or not _EXPLICIT_RE.search(text):
            return False
        # Store the complete sentence at slightly higher importance; the AI may refine it later.
        cat = "identity" if re.search(r"(我叫|叫我|我是)", text) else "preference"
        if re.search(r"(别|不要|禁止)", text):
            cat = "boundary"
        return self.upsert(text, category=cat, importance=0.85, source="user_explicit")
    ...
    def to_context_blob(self) -> dict[str, Any]:
        ...
        return {
            ...
            "facts": lines,
            "policy": (
                "Stable facts only. Use them for continuity. "
                "Write via params.remember when user states durable info; "
