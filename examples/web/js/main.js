...
(() => {
  ...
  const I18N = {
    en: {
      ...
      "banner.cta": "Install",
      "hero.subtitle":
        "Local TUI for auditable coding-agent sessions.<br />Clarify → Plan → Execute → Verify ⇄ Repair → Summarize",
      ...
      "install.copy": "Copy",
      "install.hint":
        "New terminal → <code>ton doctor</code>. <code>go install</code>: put <code>$(go env GOPATH)/bin</code> on <code>PATH</code>.",
      "cta.start": "Install",
      "cta.guide": "Guide",
      ...
      "terminal.response": "Requirements ready. Confirm Ready, then /start.",
      "terminal.f1": "🎯 Clarify + Ready — lock scope before run",
      "terminal.f2": "📋 Milestone TUI · audit trail in .ton/",
      "terminal.f3": "🔁 Verify ⇄ Repair — bounded, recoverable",
      "terminal.f4": "🔌 Drivers: OpenCode / Claude / Cursor / fake",
      "drivers.body":
    ...
    zh: {
      ...
      "banner.cta": "安装",
      "hero.subtitle":
        "本地 TUI，编排可审计的 coding-agent 会话。<br />Clarify → Plan → Execute → Verify ⇄ Repair → Summarize",
      ...
      "install.copy": "复制",
      "install.hint":
        "新开终端 → <code>ton doctor</code>。<code>go install</code>：把 <code>$(go env GOPATH)/bin</code> 加入 <code>PATH</code>。",
      "cta.start": "安装",
      "cta.guide": "指南",
      "features.title": "一套闭环，多种驱动",
      "terminal.aria": "TON TUI 预览",
      "terminal.prompt": "登录页：暗色 + 无障碍",
      "terminal.response": "需求已就绪。确认 Ready，然后 /start。",
      "terminal.f1": "🎯 Clarify + Ready — 开跑前锁范围",
      "terminal.f2": "📋 Milestone TUI · .ton/ 审计轨迹",
      "terminal.f3": "🔁 Verify ⇄ Repair — 有界可恢复",
      "terminal.f4": "🔌 驱动：OpenCode / Claude / Cursor / fake",
      "drivers.body":
