...
func (m Model) useTodoSidebar(width int) bool {
	if !m.showTodos || width < dualColumnMinWidth {
		return false
	}
	if len(m.todos.Items) > 0 {
		return true
	}
	// Keep the sidebar during planning so the UI does not look empty while the
	// agent is still writing todos.json (common with agent_plan=true).
	return m.session.Phase == domain.PhasePlanning
}
...
func clarifyContent(state clarify.ReqState, fallback string) string {
	...
	// Soft status only — gaps coach the clarify loop; hard gate is /start.
	if clarify.DocsReady(&state) && !state.Readiness.Ready {
		content.WriteString(sectionStyle.Render("Still aligning"))
		gaps := state.Readiness.Gaps
		if len(gaps) == 0 {
			gaps = []string{"keep refining requirements/design together"}
		}
		for _, g := range gaps {
			content.WriteString("\n")
			content.WriteString(mutedStyle.Render("  - "))
