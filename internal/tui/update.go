...
func (m Model) submit(input string, chatID int) (tea.Model, tea.Cmd) {
	...
	if isTerminalPhase(m.session.Phase) {
		...
		// Any non-slash message after Done/Aborted reopens clarify — no keyword sniffing.
...
func (m Model) runCommand(command command) (tea.Model, tea.Cmd) {
	...
	case commandStart:
		...
		if command.argument == "force" {
			chatID = m.rememberSlashTurn("/start --force")
		}
		m.milestoneLog = nil
		m.showTodos = true
		force := command.argument == "force"
		return m.beginBusy(func() tea.Msg {
			err := m.controller.Start(context.Background(), force)
