...
func (c *SessionController) Clarify(ctx context.Context, input string) (string, error) {
	...
	c.mu.Lock()
	// Invite /start only when drafts are thick AND LLM long-run grade passes.
	if clarify.LongRunReady(&c.state) {
	...
	runPreflight := c.cfg.Orchestrate.ReadyPreflight && !state.Acceptance.AllowNoGate && (clarify.DocsReady(&state) || forcePreflight)
	if runPreflight {
...
// Start is the only user-facing gate: PrepareStart, then plan+execute.
// force skips the LLM readiness.ready check (docs must still be adequate).
func (c *SessionController) Start(ctx context.Context, force bool) error {
	c.mu.Lock()
	...
	if resume.Kind == orch.ResumeRestoreUI && !clarify.DocsReady(&c.state) {
		c.mu.Unlock()
		return fmt.Errorf("session restored; finish clarification before /start")
	}

	c.ensureFallbackDefaults()
	if err := clarify.PrepareStart(&c.state, force); err != nil {
		c.mu.Unlock()
		return fmt.Errorf("not ready for /start: %w", err)
	}
	c.session.Phase = domain.PhaseReadyToStart
	c.session.UpdatedAt = time.Now().UTC().Format(time.RFC3339Nano)
	sessionSnap := *c.session
	stateSnap := c.state
	if c.cancel != nil {
		c.mu.Unlock()
		return fmt.Errorf("session is already running")
	...
	_ = c.store.CreateSession(sessionSnap)
	_ = c.store.SaveClarifyArtifacts(sessionSnap.ID, stateSnap)
	_ = c.upsertIndex(sessionSnap)
	...
	runner := orch.SessionRunner{
		...
		Executor: execute.Executor{
			...
			ExtraEnv:        c.browserExtraEnv(),
			HeadlessBrowser: c.cfg.Browser.Headless,
...
// Uses an isolated backend session (never touches session.BackendSessionID) so plan-time
// agent chatter cannot bleed into Execute, and UI can stay on Planning until todos exist.
func (c *SessionController) runAgentPrompt(ctx context.Context, cwd, prompt string) (string, error) {
	...
	tonSession := ""
	if c.session != nil {
		tonSession = c.session.ID
	}
	c.mu.RUnlock()
	if agent == nil {
		return "", fmt.Errorf("no agent backend")
	}
	// Isolated plan session — do not reuse or overwrite the execute BackendSessionID.
	planSID := ""
	if tonSession != "" {
		planSID = "plan-" + tonSession
	}
	ensured, err := agent.EnsureSession(ctx, cwd, planSID)
	if err != nil {
		return "", err
	}
	...
	c.emit("Planning: agent writing todos.json…")
	planPrompt := prompt
	if block := browserenv.PromptBlock(c.cfg.Browser.Headless); block != "" {
		planPrompt = block + "\n" + prompt
	}
	...
	for ev := range events {
		...
		// Keep the TUI alive during long plan-agent runs (otherwise Progress freezes on Planning).
		if time.Since(lastBeat) >= 3*time.Second {
			c.emit("Planning: agent drafting todos…")
			lastBeat = time.Now()
		}
	}
...
// browserExtraEnv returns Playwright/MCP headless env when browser.headless is enabled (default).
func (c *SessionController) browserExtraEnv() []string {
	if c == nil || !c.cfg.Browser.Headless {
		return nil
	}
	return browserenv.EnvPairs()
}
