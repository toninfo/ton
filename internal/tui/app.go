...
func (c *SessionController) Clarify(ctx context.Context, input string) (string, error) {
	...
	// 纯问候且尚无任务时跳过 conductor，直接轻量 LLM 澄清。
	...
	if orchCfg.ConductClarify && !smalltalk {
	...
	}).Turn(ctx, clarify.UserInput{Text: input}, &state)
	if err != nil {
		return "", err
	}
...
func (c *SessionController) Start(ctx context.Context) error {
	...
	planFresh := resume.Kind == orch.ResumeReplan || len(c.todos.Items) == 0
...
func (c *SessionController) writeReport() error {
	...
	saved := *c.session
	c.mu.Unlock()
	if saved.Phase == domain.PhaseAborted {
		c.emit("Session aborted")
	} else {
		c.emit("Done")
	}
