...
func (c Clarifier) Turn(ctx context.Context, input UserInput, state *ReqState) (ClarifyOut, error) {
	if c.Client == nil {
	...
	content, _, err := c.Client.Chat(ctx, []llm.Message{
		...
		{Role: "user", Content: buildClarifyUserPrompt(string(currentState), input.Text, c.RepoContext)},
	})
	if err != nil {
