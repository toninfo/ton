...
// BrowserConfig controls whether agent/verify browser automation may show a GUI window.
type BrowserConfig struct {
	// Headless hides Playwright MCP / Chromium windows (default true for unattended ton runs).
...
func Default() Config {
	return Config{
		...
		Browser: BrowserConfig{
			Headless: true, // Playwright MCP defaults to headed; ton forces headless for unattended runs
		},
	}
}
